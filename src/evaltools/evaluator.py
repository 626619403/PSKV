...
def build_alpacaeval(
    ...
    for be in range(0, len(eval_set), bs):
        ...
        inputs_mask = inputs_mask.to(model.device)
...
def build_evalset(
    ...
    if eval_config.attacker_config is not None:
        import attacks
        eval_config.attacker_config["disable_tqdm"] = False
        attacker = attacks.build_attacker(**eval_config.attacker_config)
    ...
    for inputs in dataloader:
        ...
        if attacker is not None:
            temp = attacker.attack_embeds(
                ...
                device       = model.device if type(model) is not str else 'cuda',
            )
            if temp is None:
                advsfx_ids = None
                advsfx_embeds = None
        ...
        if _model is None:
            if type(model) is str:
                _model, _tokenizer = utils.get_model(model)
            else:
                _model = model
                _tokenizer = tokenizer
                
        if advsfx_ids is not None:
            advsfx_ids = advsfx_ids.to(_model.device)
        if advsfx_embeds is not None:
            advsfx_embeds = advsfx_embeds.to(_model.device)
        inputs_ids = inputs["prompt_ids"].to(_model.device)
        inputs_mask = inputs["prompt_mask"].to(_model.device)

        offset = (inputs["harmful_mask"].cumsum(dim=-1) == 0).sum(dim=-1)[0].item()
        target_ids = inputs["harmful_ids"][:, : offset].to(_model.device)
        assert inputs["harmful_mask"][:, : offset].sum().item() == 0

        if advsfx_ids is not None:
            inputs_ids = torch.cat([inputs_ids, advsfx_ids, target_ids], dim=1)
            inputs_embeds = _model.get_input_embeddings()(inputs_ids)
            ...
            inputs_embeds = torch.cat([
                _model.get_input_embeddings()(inputs_ids),
                advsfx_embeds,
                _model.get_input_embeddings()(target_ids),
            ], dim=1)
            inputs_mask = torch.cat([
                inputs_mask,
                torch.ones(inputs_embeds.shape[:2], dtype=torch.int64, device=_model.device),
        ...
        for k in range(eval_config.per_response_num):
            ...
            outputs_text = _tokenizer.batch_decode(outputs_ids, skip_special_tokens=True)
            outputs_text_list.append(outputs_text)

        if inputs_ids is None:
            inputs_text = None
        else:
            inputs_text = _tokenizer.batch_decode(inputs_ids, skip_special_tokens=True)

        prompt_text = _tokenizer.batch_decode(inputs["raw_prompt_ids"], skip_special_tokens=True)
        target_text = _tokenizer.batch_decode(inputs["raw_harmful_ids"], skip_special_tokens=True)
...
Answer: [/INST]''',
    ...
    def __init__(
        ...
        # self.model = AutoModelForCausalLM.from_pretrained(self.model_id, dtype=torch.bfloat16, device_map=self.device)
        self.model = AutoModelForCausalLM.from_pretrained(self.model_id, dtype=torch.bfloat16, quantization_config=BitsAndBytesConfig(), device_map="auto")
        # self.tokenizer = AutoTokenizer.from_pretrained(self.model_id, use_fast=False, truncation_side="left", padding_side="left")
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_id, use_fast=True, truncation_side="left", padding_side="left")
    @torch.no_grad()
