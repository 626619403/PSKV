...
class GCG(AttackerBase):
    ...
    def attack_embeds(
        ...
        normal_cache: List[Tuple[Tuple[torch.Tensor, torch.Tensor], ...]] = []
        cache = None
        if self.kv_cache != "None":
            cache = initialize_prefix_cache(model=model, search_width=self.search_width, 
        ...
        for step in pbar:
            ...
            for ii, be in enumerate(range(0, B, grad_bs)):
                ...
                out_logits = forward_with_cache(
                    model=model, cache_mode=self.kv_cache, cache=cache[ii] if cache is not None else None, 
                    message_embeds=message_embeds[be:ed], sfx_tar_embeds=inp_embeds,
                    attention_mask=inp_mask)[:, -(tar_len+1) : -1]
            ...
            for ii, be in enumerate(range(0, width, qry_bs)):
                ...
                for kk, be2 in enumerate(range(0, B, grad_bs)):
                    ...
                    out_logits = forward_with_cache(model=model, cache_mode=self.kv_cache, cache=cache[kk] if cache is not None else None, 
                        sfx_tar_embeds=cand_embeds[:, msg_len:], full_embeds=cand_embeds, 
                        attention_mask=cand_mask ,expand_factor=ed-be)[:, -(tar_len+1) : -1]
